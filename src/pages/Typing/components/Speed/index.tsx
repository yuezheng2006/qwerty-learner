...
export default function Speed() {
  ...
  return (
    <div className="my-card flex w-3/5 rounded-xl bg-white p-4 py-10 opacity-50 transition-colors duration-300 dark:bg-gray-800">
      <InfoBox info={`${minutesString}:${secondsString}`} description="耗时" />
      <InfoBox info={inputNumber + ''} description="字数" />
      <InfoBox info={state.timerData.wpm + ''} description="速度" />
      <InfoBox info={state.chapterData.correctCount + ''} description="正确" />
      <InfoBox info={state.timerData.accuracy + ''} description="精度" />
    </div>
  )
}
